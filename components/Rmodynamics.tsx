...
const rmodynamics = () => {
    ...
    return (
        <div className="text-[#fff]">
            ...
            <div className="flex bg-[#1A1A1A] p-5 w-[max-content]">
                                ...
                                <div className="flex mb-[2px]" key={i}>
                                    <div className="text-[12px] w-[40px] h-[30px] flex items-center">{dys[i]}</div>
                                    {
                                        t.map((item: any, index: number) => (
                                            <div key={index} onClick={() => putActiveItems([i,index])} className={`${getBorderStyle([i,index])} box-border h-[30px] w-[30px] bg-[#4F4F4F] mr-[2px] cursor-pointer ${item< 3 ? 'bg-[#4F4F4F]' : item >= 3 && item <=5 ? 'bg-[#4F2D2E]' : item > 5 && item <= 7 ? 'bg-[#76312B]' : item > 7 && item <= 8 ? 'bg-[#AF342F]' : item > 8 && item < 10 ? 'bg-[#CA3B32]' : 'bg-[#F33C17]'}`}></div>
                                        ))
                                    }
                                </div>
                            ))
