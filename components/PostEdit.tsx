...
import { uploadIpfs, formatToIpfs, formatToHttps } from "../api/ipfs";
...
const PostEdit = () => {
  ...
  const { account, connectWallet, signMessage, web3, sendTx, signer } =
    useWeb3Context();
  ...
  return (
    <div className="bg-[#1A1A1A] p-5 mt-10 mb-10">
      ...
      {images.length > 0 && <div className="flex gap-4 p-4">
        {images.map(item => <img key={item} src={formatToHttps(item)} className="w-24" />)}
      </div> }

      {videos.length > 0 && <div className="flex gap-4 p-4">
        {videos.map(item => <video key={item} src={formatToHttps(item)} className="w-48" />)}
      </div> }

      {audios.length > 0 && <div className="flex gap-4 p-4">
        {audios.map(item => <audio key={item} src={formatToHttps(item)} className="w-24" />)}
      </div> }
      ...
      <div className="flex mt-4">
        <div className="flex items-center justify-center">
          <Popover
            content={
              <UpLoadPopover
                onAudioUploaded={(hash: string) => {
                    setAudios(prev => [hash, ...prev])
                }}
                onVideoUploaded={(hash: string) => {
                    setVideos(prev => [hash, ...prev])
                }}
                onImageUploaded={(hash: string) => {
                    setImages(prev => [hash, ...prev])
                }}
              />
            }
            placement="bottom"
          >
            <Image className="mr-1 cursor-pointer" src={P1} alt="" />
          </Popover>
          ...
          <Popover content={PermitPopover} placement="bottom">
            <Image className="mr-1 cursor-pointer" src={P6} alt="" />
          </Popover>
          {/* <Image
                        className="mr-1 cursor-pointer"
                        src={P7}
        ...
        <div
          onClick={() => doPost()}
          className="flex items-center justify-center ml-[auto] bg-[#CE3900] px-4 py-1 cursor-pointer rounded-[4px]"
        >
          <span>Post</span>
        </div>
      ...
      {showModal[1] && <GifModal onCancel={() => handleShowModal(false, 1)} />}
    </div>
  );
};
